...
class FarmData:
    def __init__(self, gsa_account_file, farm_unit_file, parcel_geometries_file, top_n=4):
    ...
    def load_data(self):
        ...
        parcel_geometry_data = gpd.read_file(self.parcel_geometries_file, ignore_geometry=True)
    ...
    def generate_data(self, gsa_account_data, farm_unit_data, parcel_geometry_data):
        ...
        four_data = dict(list(data.items())[:self.top_n])
        return four_data
...
output_file = 'four_data.json'

data_processing = FarmData(gsa_account_file, farm_unit_file, parcel_geometries_file, top_n=4)
