...
class FarmData:
  ...
  def generate_data(self, gsa_account_data,farm_unit_data,parcel_geometry_data):
    ...
    account_farm_unit['remaining_%'] = np.where(
        account_farm_unit["fu_total_adjustment_af"] == 0, 
        0, 
        (account_farm_unit["fu_remain_af"] / account_farm_unit["fu_total_adjustment_af"]) * 100
    )
    account_farm_unit['allocation_used_%'] = 100 - account_farm_unit['remaining_%']
    ...
    for account_id, farm_unit_frame in farm_unit_group_by_account:
      ...
      chart_data_entries = [
        {
        "name": row["farm_unit_zone"],
        "remaining_%": row['remaining_%'],
        "allocation_used_%": row['allocation_used_%']
      }
      for _, row in farm_unit_frame.iterrows()
      ]
  ...
  def exceptional_data(self , account_farm_unit , exceptional_csv_file):
    account_farm_unit['parcels'] = account_farm_unit['parcels'].apply(lambda x: str(x))
    
    exceptional_remaining = account_farm_unit[(account_farm_unit['remaining_%'] > 100) | (account_farm_unit['remaining_%'] < 0)]
    exceptional_allocation =  account_farm_unit[(account_farm_unit['allocation_used_%'] > 100 ) | (account_farm_unit['allocation_used_%'] < 0)]

    exceptional_data = pd.concat([exceptional_remaining, exceptional_allocation]).drop_duplicates()
    exceptional_data.to_csv(exceptional_csv_file, index=False)
    print(" Exceptional data saved")
...
output_file = 'exceptional_chart_data.json'
exceptional_csv_file = "exceptional_data.csv"
...
processed_data , account_farm_unit = data_processing.generate_data(gsa_account_data, farm_unit_data, parcel_geometry_data)
data_processing.save_data(processed_data, output_file, )
data_processing.exceptional_data(account_farm_unit , exceptional_csv_file)